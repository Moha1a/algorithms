...
class _BookingsScreenState extends State<BookingsScreen> {
    ...
    if (status == 'completed' && isParty) {
      widgets.add(
        TextButton(
          ...
          child: const Text('تقييم بعد الإكمال'),
        ),
      );
    }
  ...
  Future<void> _suggestPrice(String bookingDocId) async {
    ...
    final maxWithdrawCommission = bookingAmount * 0.006;
    if (bookingType == 'withdraw' && price > maxWithdrawCommission) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('تم رفض الاقتراح: الحد الأقصى لعمولة السحب هو ${MoneyUtils.iqdWithWords(maxWithdrawCommission)}.')),
      );
      return;
    }
