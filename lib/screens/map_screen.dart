...
class _BookingMapDetailsScreenState extends State<BookingMapDetailsScreen> {
  ...
  Widget _buildCompletionSection(BuildContext context, Map<String, dynamic> booking, String status) {
    ...
    final expiresAtRaw = booking['completionCodeExpiresAt'];
        ...
        child: Column(
          ...
          children: [
            ...
            if (isRequester && status == 'awaiting_provider_code') ...[
              ...
              if (expiresAt != null) Padding(
                padding: const EdgeInsets.only(top: 6),
                child: Text('آخر صلاحية مسجلة: ${expiresAt.toLocal()}'),
              ),
            ],
            ...
            content: Column(
              ...
              children: [
                ...
                FilledButton.icon(
                  ...
                  label: const Text('توليد رمز جديد (30 ثانية)'),
                ),
                const Text(
      ...
      final ok = await showDialog<bool>(
        ...
        builder: (dialogContext) => StatefulBuilder(
          builder: (dialogContext, setStateDialog) => AlertDialog(
            title: const Text('تقييم الطرف الآخر'),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Row(
